...
static void on_clock2_gesture(lv_event_t *) {
    ...
    if (dir == LV_DIR_LEFT)  nav_to(scr_clock2, scr_weather,  LV_SCR_LOAD_ANIM_MOVE_LEFT);
    if (dir == LV_DIR_RIGHT) nav_to(scr_clock2, scr_settings, LV_SCR_LOAD_ANIM_MOVE_RIGHT);
    if (dir == LV_DIR_TOP)   nav_to(scr_clock2, scr_nav,      LV_SCR_LOAD_ANIM_MOVE_TOP);
}

static void on_clock2_tap(lv_event_t *) {
    if (lv_scr_act() != scr_clock2) return;
    nav_to(scr_clock2, scr_nav, LV_SCR_LOAD_ANIM_MOVE_TOP);
}
